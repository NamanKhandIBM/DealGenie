...
- **Fork variables** — `getForkVariables()` generates option lists **anchored to the actual quoted value** using a geometric ×5 scale (2 steps below, current value, 2 steps above), so a user who quoted 100 devices sees 4/20/100/500/2,500 — not a static generic list
- **Scenario price** — `computeScenarioPrice()` has been fully audited against every product's quote flow. All add-ons, secondary use cases (Vault SSH/Transit/KMSE), cloud-spend scoping (Turbonomic), B2B transactions (webMethods), Dedicated PoPs and China DNS (NS1), and cross-sell answer keys (Verify) are passed through so the Running Total always matches the original quote price
- **Add-on panel** — persistent checkboxes for binary add-ons (Verify: SMS/HAG/Vanity/NonProd; Vault: NonProd/KMIP; NS1: DDoS protection); live delta shown per toggle
...
After any quote result, the UI renders visual attach cards (one per recommended target) that include:
...
All work is on `main`. The `cross-plays` branch is stale (behind main by 4+ commits) and should not be used.

Current commit: `c321104` — Complete `computeScenarioPrice` input audit across all 9 products.
